...
class Translator(object):
    ...
    def _load_models(self, process_id, sess):
        ...
        for i, options in enumerate(self._options):
            with tf.variable_scope("model%d" % i) as scope:
                # ===== 08/23 KP Transformer model logic ====
    ...
    def _start_worker(self, process_id):
        ...
        while True:
            ...
            # shutdown方法内，将 None 作为元素填入队列，这里获取之后进程就停止循环
    ...
    def translate(self, source_segments, translation_settings):
        ...
        logging.info('=== outputs: {}, len(outputs): {}'.format(outputs, len(outputs)))
        translations = []
        for i, beam in enumerate(outputs):
            ...
            logging.info('===0000000 translation_settings: {}, translation_settings.n_best: {}, self._num_to_target: {}'.format(translation_settings, translation_settings.n_best, len(self._num_to_target)))
            if translation_settings.n_best is True:
                ...
                target_words = util.seq2words(best_hypo, self._num_to_target,
                                              join=False)
                logging.info('===2222222 sent: {}, target_words: {}'.format(best_hypo, target_words))
                translation = Translation(sentence_id=i,
                                            source_words=source_segments[i],
                                            target_words=target_words,
                                            score=cost)
